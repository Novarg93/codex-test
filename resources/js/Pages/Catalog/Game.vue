  ...
  <DefaultLayout>
    <section class="w-[90%] 2xl:w-[75%] mx-auto py-8 md:py-12 lg:py-16">
      ...
      <header class="my-6">
        ...
        <p v-if="category?.short || category?.description || game?.description"
   class="text-muted-foreground mt-2">
  {{ category?.short ?? category?.description ?? game?.description }}
</p>
      </header>
      ...
      <div class="grid grid-cols-12 gap-6">
        ...
        <aside class="col-span-12 md:col-span-3 space-y-2">
          ...
          <Link v-for="c in categories" :key="c.id" :href="route('categories.show', [game.slug, c.slug])"
        ...
        <main class="col-span-12 md:col-span-9">
          <div class="grid gap-4 grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
            <article v-for="p in products.data" :key="p.id"
              ...
              <Link
                ...
                class="mt-2 inline-block text-primary hover:underline text-sm">View</Link>
            </article>
          </div>

          <!-- SEO long description -->
          <div v-if="seo.seo_text" class="prose prose-invert max-w-none mt-10">
            <details>
              <summary class="cursor-pointer select-none">Read more</summary>
              <div v-html="seo.seo_text" />
            </details>
          </div>

          <!-- pagination -->
          <div v-if="products.links?.length" class="mt-8 flex gap-2 flex-wrap">
            <Link v-for="l in products.links" :key="l.url ?? l.label" :href="l.url || '#'"
