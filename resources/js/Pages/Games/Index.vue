  ...
  <DefaultLayout>
    <section class="w-[90%] 2xl:w-[75%] mx-auto py-10 md:py-16 lg:py-20">
      ...
      <div class="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        <article v-for="g in games" :key="g.id" class="border border-border rounded-xl p-3 hover:shadow transition">
          <img v-if="g.image_url" :src="g.image_url" alt="" class="w-full h-36 object-cover rounded-lg mb-3" />
          <h3 class="font-medium mb-2 line-clamp-2">{{ g.name }}</h3>

          <Link :href="route('games.show', g.slug)" class="text-primary hover:underline text-sm">
            Open
          </Link>
        </article>
      </div>
